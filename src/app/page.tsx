...
import { TaxationSVG } from "@/lib/svg/taxation";
import { AuditingSVG } from "@/lib/svg/auditing";
import { BusinessConsultantSVG } from "@/lib/svg/businessconsultant";
import { IpoListingSVG } from "@/lib/svg/ipolisting";
...
export default function Home() {
  ...
  return (
    <main className={`relative text-black min-h-screen overflow-x-hidden ${isLoading ? "overflow-hidden" : ""}`}>
      ...
      <motion.div
        ...
        id="hero"
        className="landing-page relative top-0 min-h-full bg-background w-full"
      >
        <motion.nav
          initial={{ y: -100 }}
          animate={{ y: 0 }}
            ...
            <div className="nc sizer flex justify-between h-12">
              <div className="left-nav flex items-center space-x-10 text-[16px]">
                <div className="nav-log text-white flex items-center space-x-2">
                  <MapPin /> <p>Chennai, India</p>
                </div>
                <div className="nav-mail text-white flex items-center space-x-2">
                  <Mail /> <p>contact@bizniz.com</p>
                </div>
              </div>
              <div className="right-nav flex items-center space-x-2">
                <Link href="#">
                  <svg
                    width="30"
                    height="38"
        ...
        <section>
          <div className="sizer">
            <div className="cards w-full justify-center items-center m-auto">
              {services.map((service, index) => (
                <div
                  key={index}
                  className="card flex max-w-4xl justify-between items-center m-auto gap-5"
                >
                  {index % 2 === 0 ? (
                    <motion.div
                  ...
                  <div className="card-content">
                    <motion.h3
                      ...
                      className="leading-normal font-bold text-3xl"
                    >
                      {service.title}
                    </motion.h3>
                    <motion.p
                      ...
                      viewport={{ once: true }}
                      transition={{ duration: 1, delay: 0.5 }}
                      className="leading-normal text-lg font-light max-w-lg"
                    >
                      {service.description}
                    </motion.p>
                  </div>
